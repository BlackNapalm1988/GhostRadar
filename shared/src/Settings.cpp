...
static const uint8_t VARIANCE_STEP_COUNT = sizeof(VARIANCE_STEPS) / sizeof(VARIANCE_STEPS[0]);
...
ComplicationType Settings_parseComplicationType(const String& s) {
  if (s.equalsIgnoreCase("off") || s.equalsIgnoreCase("none")) return ComplicationType::None;
  if (s.equalsIgnoreCase("temperature_c"))     return ComplicationType::TemperatureC;
  if (s.equalsIgnoreCase("humidity_percent"))  return ComplicationType::HumidityPercent;
  if (s.equalsIgnoreCase("battery_percent"))   return ComplicationType::BatteryPercent;
  if (s.equalsIgnoreCase("wifi_strength_percent")) return ComplicationType::WifiStrengthPercent;
  return ComplicationType::None;
}

const char* Settings_complicationTypeToString(ComplicationType type) {
  switch (type) {
    case ComplicationType::TemperatureC:       return "temperature_c";
    case ComplicationType::HumidityPercent:    return "humidity_percent";
    case ComplicationType::BatteryPercent:     return "battery_percent";
    case ComplicationType::WifiStrengthPercent:return "wifi_strength_percent";
    case ComplicationType::None: default:      break;
  }
  return "off";
}
...
float Settings_clampVariance(float v) {
  if (v < VARIANCE_STEPS[0]) return VARIANCE_STEPS[0];
  if (v > VARIANCE_STEPS[VARIANCE_STEP_COUNT - 1]) return VARIANCE_STEPS[VARIANCE_STEP_COUNT - 1];
  return v;
}
...
uint16_t Settings_clampHeartbeat(uint16_t bpm, uint16_t minBpm, uint16_t maxBpm) {
...
LoggingLevel Settings_parseLoggingLevel(const String& s) {
  if (s.equalsIgnoreCase("debug")) return LOG_LEVEL_DEBUG;
  if (s.equalsIgnoreCase("warn") || s.equalsIgnoreCase("warning")) return LOG_LEVEL_WARN;
  if (s.equalsIgnoreCase("error")) return LOG_LEVEL_ERROR;
...
float Settings_stepVariance(int8_t direction) {
  ...
  if (next < 0) next = 0;
  if (next >= VARIANCE_STEP_COUNT) next = VARIANCE_STEP_COUNT - 1;
