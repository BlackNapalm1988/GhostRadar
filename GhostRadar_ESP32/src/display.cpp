...
static const uint16_t COL_TEXT_BASE        = 0xC618; // soft light grey (not pure white)
...
static const uint8_t BRIGHTNESS_LEVEL_COUNT = sizeof(BRIGHTNESS_SCALES) / sizeof(BRIGHTNESS_SCALES[0]);
...
static UiStyle uiStyle = {
  { COL_BG_BASE, COL_PANEL_BASE, COL_TEXT_BASE, COL_TEXT_DIM_BASE, COL_ACCENT_RED_BASE, COL_ACCENT_SOFT_BASE },
  { 0x0000, 0x1082, 0xBDF7, 0x7BEF, 0xF980, 0x9000 }, // night variant: warmer, softer
  false,
  BRIGHTNESS_SCALES[2]
};

Adafruit_ILI9341 tft(LCD_CS, TFT_DC, TFT_RST);
static char lastDisplayedLetter = 0;
static uint8_t currentBrightnessLevel = 2;

#if defined(TFT_BL)
static const int DISPLAY_BL_PIN = TFT_BL;
#elif defined(TFT_LED)
static const int DISPLAY_BL_PIN = TFT_LED;
#elif defined(BL_PIN)
static const int DISPLAY_BL_PIN = BL_PIN;
#else
static const int DISPLAY_BL_PIN = -1;
#endif
...
static unsigned long heartbeatScrollIntervalMs = HEARTBEAT_SCROLL_INTERVAL_BASE_MS;
...
static void pushWordHistory(const String& w) {
  ...
  wordHistory[0] = w;
  if (wordHistoryCount < WORD_HISTORY_MAX) wordHistoryCount++;
}

static void computeLayout() {
  // Asymmetric layout: radar anchored to the left, recents on the right.
  const int LEFT_BADGE_W = 0;   // letter badge floats over radar, not reserving column space
  ...
  layout.moduleRightW = max(RIGHT_PANEL_MIN, INNER_W - RADAR_W - 4); // 4px gutter between radar and recents

  // Keep canvas dimensions tied to the allocated radarCanvas (RADAR_W/H) to avoid stride mismatches.
...
static uint16_t applyBrightness(uint16_t color) {
  return uiStyle.scale(color);
}
...
static void ensureBacklightConfigured() {
  if (backlightConfigured || DISPLAY_BL_PIN < 0) return;
...
static bool heartbeatReady() {
  return heartbeatCanvas.getBuffer() != nullptr;
}

static void updateHeartbeatInterval(uint16_t bpm) {
  uint16_t clamped = Settings_clampHeartbeat(bpm);
  heartbeatScrollIntervalMs = (uint32_t)HEARTBEAT_SCROLL_INTERVAL_BASE_MS * HEARTBEAT_SCROLL_BPM_BASE / (clamped == 0 ? 1 : clamped);
...
static void drawHeaderStatusArea() {
  ...
  drawWifiIcon(iconX, iconY, iconSize, statusColor(statusWifiActive, wifiPulse), bg);
  iconX -= iconSize + 6;
  drawLogIcon(iconX, iconY, iconSize, statusColor(statusLoggingEnabled, false), bg);
  iconX -= iconSize + 6;
...
static void drawWordHistoryPanel() {
  ...
  if (labelX < areaX + 4) labelX = areaX + 4;
  tft.setTextColor(colAccent());
...
static void drawHeader() {
  uint16_t bg = colPanel();
  int radius = 6;
  tft.fillRoundRect(layout.headerX, layout.headerY, layout.headerW, layout.headerH, radius, bg);
...
static void drawModules() {
  ...
  if (layout.moduleLeftW > 0) {
    tft.fillRect(layout.mainX, layout.mainY, layout.moduleLeftW, layout.mainH, bg);
  }
  tft.fillRect(layout.mainX + layout.mainW - layout.moduleRightW, layout.mainY, layout.moduleRightW, layout.mainH, bg);

  if (ModuleLeft_enabled()) {
    ModuleLeft_draw(tft, layout.mainX, layout.mainY, layout.moduleLeftW, layout.mainH);
  }
  if (ModuleRight_enabled()) {
    ModuleRight_draw(tft, layout.mainX + layout.mainW - layout.moduleRightW, layout.mainY, layout.moduleRightW, layout.mainH);
  }
}
...
void Display_drawStaticFrame() {
  ...
  tft.fillRect(layout.radarX, layout.radarY, layout.radarW, layout.radarH, colBg());
  tft.fillRect(layout.heartbeatX, layout.heartbeatY, layout.heartbeatW, layout.heartbeatH, colBg());
  heartbeatReset();
  drawLetterBadge(0);
...
void Display_setSdPresent(bool present) {
  ...
  if (tftReady) drawHeaderStatusArea();
}
...
void Display_displayLetter(char l) {
  if (l == lastDisplayedLetter) return;
...
void Display_drawOverlayWord() {
  if (overlayWord.length() == 0) return;
  ...
  unsigned long now = millis();
  if (overlayFadeStartMs == 0) overlayFadeStartMs = now;
...
void Display_heartbeatStep(char letterOrZero) {
  ...
  if (letterOrZero == 0) return;  // only draw real letters
  // Drop oldest if at capacity
  if (heartbeatEntryCount >= HEARTBEAT_MAX_ENTRIES) {
    memmove(heartbeatEntries, heartbeatEntries + 1, (HEARTBEAT_MAX_ENTRIES - 1) * sizeof(HeartbeatEntry));
  ...
  if (heartbeatEntryCount > 0) {
    ...
    if (lastX > startX) startX = lastX + HEARTBEAT_SPACING;
  }
...
void Display_updateHeartbeat() {
  ...
  if (now - lastHeartbeatStepMs >= heartbeatScrollIntervalMs) {
    ...
    for (int i = 0; i < heartbeatEntryCount; i++) {
      ...
      if (heartbeatEntries[i].x < -HEARTBEAT_SPACING) continue;
  ...
  heartbeatCanvas.fillRect(heartbeatDirtyX0, 0, heartbeatDirtyX1 - heartbeatDirtyX0, HEARTBEAT_H, colBg());
  uint16_t lineC = colTextDim();
  int midY = HEARTBEAT_H / 2;
  heartbeatCanvas.drawLine(heartbeatDirtyX0, midY, heartbeatDirtyX1, midY, lineC);
...
static void drawVarianceRow(int top, int height, float varianceScale) {
  ...
  for (uint8_t i = 0; i < stopCount; i++) {
    ...
    uint16_t c = (fabsf(stops[i] - clamped) < 0.01f) ? colAccent() : colTextDim();
...
void Display_drawSettingsScreen(const DeviceSettings& s) {
  ...
  uint8_t clampedBrightness = Settings_clampBrightness(s.brightnessLevel, BRIGHTNESS_LEVEL_COUNT - 1);
  uint8_t dictIdx = s.dictionaryIndex;
  uint8_t dictCount = Dictionary_getCount();
  if (dictCount > 0 && dictIdx >= dictCount) dictIdx = dictCount - 1;
