...
String formatTimestamp() {
  ...
  char fallback[28];
  snprintf(fallback, sizeof(fallback), "0000-00-00T00-00-%lu", (unsigned long)(ms / 1000UL));
...
void ensureDirectories() {
  ...
  const char* dirs[] = { CONFIG_DIR, LOGS_DIR, SESSIONS_DIR, DICTIONARY_DIR, UI_DIR };
...
bool saveDefaultSystemConfigIfMissing() {
  ...
  if (SD.exists(SYSTEM_CONFIG_PATH)) return true;
  ...
  logging["level"] = Settings_loggingLevelToString((LoggingLevel)s.loggingLevel);
  ...
  writeComplication(complications["bottom_left"].to<JsonObject>(), s.ui.bottomLeft);
  writeComplication(complications["bottom_right"].to<JsonObject>(), s.ui.bottomRight);
...
bool loadSystemConfig() {
  ...
  DeviceSettings &s = Settings_get();
  s.brightnessLevel = Settings_clampBrightness(doc["brightness"] | s.brightnessLevel, 3);
  s.heartbeatBpm = Settings_clampHeartbeat(doc["heartbeat_speed"] | s.heartbeatBpm);

  JsonVariant logging = doc["logging"];
  if (!logging.isNull()) {
    s.loggingEnabled = logging["enabled"] | s.loggingEnabled;
    const char* lvl = logging["level"] | Settings_loggingLevelToString((LoggingLevel)s.loggingLevel);
...
void logEvent(const String &line) {
  if (!sdAvailable) return;
  if (!Settings_get().loggingEnabled) return;
  if (!levelAllows(LOG_LEVEL_INFO)) return;
...
void logSessionLine(const String &line) {
  if (!sdAvailable || !loggingActive) return;
